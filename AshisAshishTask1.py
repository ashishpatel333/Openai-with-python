...
openai.api_key = 'sk-None-yoABSsaJvanxScrgTrmGT3BlbkFJ0AxZBhpdaTJiclg91CD5'
...
try:
    with open(input_file, mode='r', encoding='utf-8') as infile, open(output_file, mode='w', encoding='utf-8', newline='') as outfile:
        ...
        writer.writeheader()
        for row in reader:
            description = row['Description']  #  CSV column name
...
try:
    with open(output_file, mode='r', encoding='utf-8') as infile:
        reader = csv.DictReader(infile)
        relevant_companies = [row for row in reader if any(keyword in row['Categorized Services'].lower()
        for keyword in ['Email','Bulk Email','United States','email sending', 'email delivery'])]
